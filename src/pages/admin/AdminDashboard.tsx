...
import { Plus, Trash, Users, Film, LogOut, MessageSquare, Check, Video, Settings, List, Star, Link as LinkIcon, Image as ImageIcon, Loader2 } from 'lucide-react';
...
export const AdminDashboard = () => {
  const { 
    ...
    suggestions, deleteSuggestion, uploadFile
  } = useStore();
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, field: 'posterUrl' | 'videoUrl' | 'trailerUrl') => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        const path = `uploads/${field}/${Date.now()}_${file.name}`;
        const url = await uploadFile(file, path);
        
        setNewContent(prev => ({ ...prev, [field]: url }));
        
        // If it's a video, try to get duration (this works with remote URLs too in modern browsers mostly)
        if (field === 'videoUrl') {
  ...
  const handleEpisodeFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files && e.target.files[0]) {
          const file = e.target.files[0];
          setUploading(true);
          try {
             const path = `uploads/episodes/${Date.now()}_${file.name}`;
             const url = await uploadFile(file, path);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // We don't need to generate ID here, Firestore does it
    await addContent({
      ...newContent,
      addedAt: Date.now(),
          ...
          <div className="bg-vtv-card w-full max-w-lg rounded-xl border border-vtv-purple p-6 max-h-[90vh] overflow-y-auto animate-in zoom-in-95">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="border border-gray-700 rounded p-4 border-dashed hover:border-vtv-neon transition-colors relative group">
                ...
                {newContent.posterUrl && <p className="text-xs text-green-500 mt-1 truncate">Fichier chargé</p>}
              </div>
              ...
              {newContent.type !== ContentType.SERIES ? (
                <div className="space-y-4">
                  ...
                  <div className="bg-black/20 p-4 rounded border border-gray-800">
                      ...
                      <button type="button" onClick={addEpisodeToNewContent} disabled={!tempEp.title || !tempEp.videoUrl || uploading} className="w-full bg-gray-700 hover:bg-white hover:text-black py-2 rounded text-xs font-bold transition disabled:opacity-50">
                          {uploading ? 'ENVOI...' : 'AJOUTER ÉPISODE'}
                      </button>
              ...
              <div className="flex gap-4 pt-2">
                  <button type="submit" disabled={uploading} className="flex-1 bg-vtv-neon text-black font-bold py-3 rounded hover:bg-cyan-300 disabled:opacity-50 flex items-center justify-center gap-2">
                    {uploading ? <Loader2 className="animate-spin" /> : 'PUBLIER'}
                  </button>
